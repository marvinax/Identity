(function(window, document, Math, undef){
	...
	var setTexture = function(canvas){
	...
	$(document).ready( function(){
		...
		canvas = new Tautology.Canvas('viewport');
		canvas.init();

		canvas.canvas.backgroundColor = 'rgba(255,255,255, 1)';
		...
		$(".thumb").on('click', function(){
			var url = $(this).children().first().children().first().attr('src');
			canvas.add(url);
		});
	});
