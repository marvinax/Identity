            ...
            output_breaks, emxArray_real_T *output_coefs)
{
  ...
  emxArray_real_T *t4_breaks;
  emxArray_real_T *t4_coefs;
  int szdvdf[2];
  int pgm1;
  emxArray_real_T *dx;
  int pgm2;
  ...
  emxInit_real_T(&t4_breaks, 2);
  c_emxInit_real_T(&t4_coefs, 3);
  if (x->size[0] == 2) {
    if (has_endslopes) {
      ...
      pwchcore(x, y, y->size[0], dx, t4_breaks, t4_coefs);
      pgm1 = output_breaks->size[0] * output_breaks->size[1];
      output_breaks->size[0] = 1;
      output_breaks->size[1] = t4_breaks->size[1];
      emxEnsureCapacity((emxArray__common *)output_breaks, pgm1, (int)sizeof
                        (double));
      pgm2 = t4_breaks->size[0] * t4_breaks->size[1];
      emxFree_real_T(&dx);
      for (pgm1 = 0; pgm1 < pgm2; pgm1++) {
        output_breaks->data[pgm1] = t4_breaks->data[pgm1];
      }

      pgm1 = output_coefs->size[0] * output_coefs->size[1] * output_coefs->size
        [2];
      output_coefs->size[0] = t4_coefs->size[0];
      output_coefs->size[1] = t4_coefs->size[1];
      output_coefs->size[2] = 4;
      emxEnsureCapacity((emxArray__common *)output_coefs, pgm1, (int)sizeof
                        (double));
      pgm2 = t4_coefs->size[0] * t4_coefs->size[1] * t4_coefs->size[2];
      for (pgm1 = 0; pgm1 < pgm2; pgm1++) {
        output_coefs->data[pgm1] = t4_coefs->data[pgm1];
      }
    } else {
      for (pgm1 = 0; pgm1 < 2; pgm1++) {
    ...
    if ((x->size[0] == 3) && (!has_endslopes)) {
      ...
      b_pwchcore(x, y, yoffset, s, dx, dvdf, t4_breaks, t4_coefs);
      pgm1 = output_breaks->size[0] * output_breaks->size[1];
      output_breaks->size[0] = 1;
      output_breaks->size[1] = t4_breaks->size[1];
      emxEnsureCapacity((emxArray__common *)output_breaks, pgm1, (int)sizeof
  ...
  emxFree_real_T(&t4_coefs);
  emxFree_real_T(&t4_breaks);
}
